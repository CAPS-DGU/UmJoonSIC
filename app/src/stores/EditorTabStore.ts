...
export const useEditorTabStore = create<EditorTabState>((set, get) => ({
    ...
    addTab: (newTab) => set((state) => {
        ...
        if (exists) {
            return { tabs: state.tabs.map((tab) => ({ ...tab, isActive: tab.filePath === newTab.filePath ? true : false })) };
        }
        // 새로운 탭 추가하고 활성화 설정
        return {
    ...
    setActiveTab: (idx) => set((state) => ({ tabs: state.tabs.map((tab) => ({
        ...tab,
